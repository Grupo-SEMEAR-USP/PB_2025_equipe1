...
class JustMove:
    def __init__(self):
        ...
        # --- Ganhos do PID (SINTONIA) ---
        self.kp = 2   # Força da correção imediata
        self.ki = 0.2  # (NOVO) Corrige o erro de regime (puxada lateral)
        self.kd = 0.2   # (NOVO) Evita oscilação (segura a inércia)
    ...
    def odom_cb(self, msg):
        ...
        # ATUALIZAÇÃO CRÍTICA: Variável de classe
    ...
    def movement_loop(self):
        ...
        while not rospy.is_shutdown():
            # Reta X
            rospy.loginfo("Reta X")
            self.move_straight(self.x_dist)
            self.move_turn_90()
            
            # Reta Y
            rospy.loginfo("Reta Y")
            self.move_straight(self.y_dist)
            self.move_turn_90()

            # Volta X
            rospy.loginfo("Reta X (Volta)")
            self.move_straight(self.x_dist)
            self.move_turn_90()
            
            # Volta Y
            rospy.loginfo("Reta Y (Volta)")
            self.move_straight(self.y_dist)
            self.move_turn_90()
    ...
    def move_straight(self, target_distance):
        ...
        # 1. Pega ponto inicial da reta (Reference Frame)
        start_x = self.current_x
        start_y = self.current_y
        
        # 2. Reseta o PID para a nova reta
        self.integral_error = 0.0
        self.last_error = 0.0
        ...
        while not rospy.is_shutdown():
            ...
            # --- CÁLCULO DO PID ---
            # Erro = Onde devia estar - Onde estou
